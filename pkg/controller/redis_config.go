...
const (
	RedisConfigKey          = "db.conf"
	RedisConfigRelativePath = "db.conf"
)
...
func (c *Controller) ensureRedisConfig(db *api.Redis) error {
	...
	if db.Spec.ConfigSecret == nil {
		...
		//finalConfig := c.makeConfig(redis)
...
func (c *Controller) checkSecret(db *api.Redis) error {
	...
	if configSecret.Labels[api.LabelDatabaseKind] != api.ResourceKindRedis ||
		configSecret.Labels[api.LabelDatabaseName] != db.Name {
...
func (c *Controller) checkUsedSecret(db *api.Redis) (error, string) {
	// Secret for Redis configuration
	...
	if _, found := configSecret.Labels[api.LabelDatabaseKind]; !found {
		return nil, string(configSecret.Data[RedisConfigKey])
	}
